  ...
  try {
    ...
    if (action === "exchange_code") {
      ...
      if (existingToken) {
        ...
        // Insert new record
        const { error } = await supabaseClient.from("google_tokens").insert(tokenPayload);
        saveError = error;
      }
    ...
    if (action === "get_valid_token") {
      ...
      if (Date.now() > expiresAt - 5 * 60 * 1000) {
        ...
        try {
          ...
          return new Response(JSON.stringify({ access_token: null, error: refreshErr.message }), {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
      }

      return new Response(JSON.stringify({ access_token: tokenRecord.access_token }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
