-- Trigger function to enforce updated publishing limits (Basic: 5, Pro: 20, Agency: 50) including purchased extra credits and admin bypass
...
BEGIN
  ...
  IF NEW.status = 'published' AND (TG_OP = 'INSERT' OR (OLD.status IS DISTINCT FROM 'published')) AND NOT COALESCE(NEW.has_been_published, false) THEN
    -- Get user plan, email, current usage, and purchased credits
    SELECT plan, email, billing_cycle_tours_used, credits INTO v_plan, v_email, v_used, v_credits
    FROM public.profiles
    WHERE id = NEW.user_id;

    -- Map plan/email to limit (Trial: 1, Basic: 5, Pro: 20, Agency: 50, Admins: 9999)
    ...
    -- Factor in purchased extra credits
    IF v_credits IS NOT NULL AND v_credits > v_limit THEN
      v_limit := v_credits;
    END IF;
