...
function SettingsPage() {
      ...
      const options = {
        ...
        handler: async function (response: any) {
          ...
          try {
            ...
            toast.success(`Welcome to the ${planName} plan! Subscription activated successfully.`, { id: verifyId });
            loadProfile();
          } catch (err: any) {
  ...
  const cancelSubscription = async () => {
    if (!window.confirm("Are you sure you want to cancel your current subscription? Your account will return to the Trial tier.")) return;
    ...
    <AppShell title="Settings" breadcrumbs={[{ label: "Dashboard", to: "/dashboard" }, { label: "Settings" }]}>
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-6 min-h-[600px] pb-12">
        ...
        <div className="w-full md:w-64 bg-slate-50 border rounded-2xl p-4 flex flex-row md:flex-col gap-1 overflow-x-auto md:overflow-x-visible shrink-0 shadow-sm h-fit">
          <div className="hidden md:block text-xs font-bold text-gray-400 uppercase tracking-widest px-3 mb-2">Settings Menu</div>
          {sidebarTabs.map((tab) => {
            const Icon = tab.icon;
                ...
                <div className="grid md:grid-cols-3 gap-8">
                  ...
                  <div className="md:col-span-2 space-y-6">
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-1">Review and manage the details provided during signup.</p>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <Label className="text-xs font-bold text-gray-500">First Name</Label>
                        <Input value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder="e.g. Prashantkumar" />
                      </div>
                      <div className="space-y-1.5">
                        <Label className="text-xs font-bold text-gray-500">Last Name</Label>
                        <Input value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder="e.g. Yadav" />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        ...
                        <Input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="e.g. vista360digital" />
                      </div>
                      <div className="space-y-1.5">
                        <Label className="text-xs font-bold text-gray-500">Account / Business Name</Label>
                        <Input value={companyName} onChange={(e) => setCompanyName(e.target.value)} placeholder="e.g. Vista360 Digital" />
                      </div>
                    </div>

                    <div className="space-y-1.5">
                      <Label className="text-xs font-bold text-gray-500">Email Address (Read-only)</Label>
                      <Input value={user?.email ?? ""} disabled className="bg-gray-50 cursor-not-allowed text-gray-500 font-medium" />
                    </div>
                    ...
                    <div className="pt-2">
                      <Button onClick={saveBasic} disabled={saving} className="bg-[#0277bd] hover:bg-[#0266a1] text-white px-6 font-bold shadow-md">
                    ...
                    <div className="border-t pt-6 mt-8 space-y-4">
                      <div>
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">Keep your account secure by resetting your password here.</p>
                      </div>
                      ...
                      <form onSubmit={handlePasswordChange} className="space-y-4 max-w-md">
                        ...
                        <Button type="submit" disabled={updatingPassword} variant="outline" className="border-gray-300 hover:bg-gray-50 font-bold">
                  ...
                  <div className="flex flex-col items-center border-l pl-8 space-y-6">
                    <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">Company Logo / Avatar</span>
                    ...
                    <div className="w-full space-y-4 pt-4 border-t">
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <div className="text-[10px] text-gray-400">Dim application UI interface.</div>
                        </div>
              ...
              {activeTab === "branding" && (
                <div className="space-y-6 max-w-3xl">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">Configure company profiles, logo, and links for virtual tour embedding.</p>
                  </div>
                  ...
                  <div className="bg-slate-50 border p-5 rounded-2xl flex flex-col sm:flex-row gap-5 items-center">
                    ...
                    <div className="space-y-2 text-center sm:text-left flex-1">
                      ...
                      <p className="text-[11px] text-gray-400">This logo will represent your company across virtual tours, watermarks, custom embedding portals, and business emails.</p>
                      <div className="flex justify-center sm:justify-start gap-2 pt-1">
                        ...
                        {logoUrl && (
                          <button onClick={removeLogo} className="border border-red-200 text-red-600 hover:bg-red-50 rounded px-3 py-1.5 text-xs font-bold shadow-sm">
                            Remove
                          </button>
                        )}
                      </div>
                  ...
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      <Label className="text-xs font-bold text-gray-500">Business / Portfolio Website URL</Label>
                      <div className="relative">
                        <Globe className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                        <Input 
                          value={websiteUrl} 
                          onChange={(e) => setWebsiteUrl(e.target.value)} 
                          placeholder="e.g. https://www.vista360digital.com" 
                          className="pl-9"
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1.5">
                        <Label className="text-xs font-bold text-gray-500">LinkedIn Company URL</Label>
                        <div className="relative">
                  ...
                  <div className="pt-2">
                    <Button onClick={saveBranding} disabled={saving} className="bg-[#0277bd] hover:bg-[#0266a1] text-white px-6 font-bold shadow-md">
              ...
              {activeTab === "billing" && (
                <div className="space-y-8">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">Manage your active plans, billing history, invoicing details, and upgrade securely.</p>
                  </div>
                  ...
                  <div className="bg-[#f0f9ff] border border-blue-100 rounded-2xl p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                      <span className="text-[10px] font-bold text-blue-500 uppercase tracking-widest block mb-1">Active Plan</span>
                      <div className="text-2xl font-extrabold text-blue-900 capitalize flex items-center gap-2">
                        {profile?.plan ?? "trial"} Tier
                        <span className="text-xs bg-blue-100 text-blue-700 px-2.5 py-0.5 rounded-full border border-blue-200">Active</span>
                      </div>
                      {profile?.trial_ends_at && profile.plan === "trial" && (
                        <p className="text-xs text-blue-600 mt-1">Free Trial ends on <strong>{formatDateIN(profile.trial_ends_at)}</strong></p>
                      )}
                    </div>
                    {profile?.plan !== "trial" && (
                      <Button variant="outline" onClick={cancelSubscription} className="border-red-200 hover:bg-red-50 text-red-600 hover:text-red-700 font-bold">
                        Cancel Subscription
                      </Button>
                    )}
                  </div>
                  ...
                  {/* Razorpay Plans Grid */}
                  <div className="space-y-4">
                    <h3 className="text-sm font-bold text-gray-700 uppercase tracking-wider">Available Subscription Upgrades</h3>
                    <div className="grid md:grid-cols-3 gap-5">
                      
                      {/* Basic Plan card */}
                      <div className={`border rounded-2xl p-5 flex flex-col justify-between transition-all relative ${
                        profile?.plan === 'basic' ? 'border-[#0277bd] bg-blue-50/10 shadow-md ring-2 ring-[#0277bd]/20 scale-102' : 'hover:border-gray-300'
                      }`}>
                        <div className="space-y-2">
                          <div className="text-base font-extrabold text-gray-800">Basic Tier</div>
                          <p className="text-[11px] text-gray-400 leading-snug">Perfect for single agency builders or small virtual tour creators.</p>
                          ...
                          <ul className="text-xs text-gray-500 space-y-1.5 pt-2 border-t font-medium">
                            <li className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-green-500 shrink-0" /> 1 user</li>
                            <li className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-green-500 shrink-0" /> 5 tours</li>
                            <li className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-green-500 shrink-0" /> 50 photos per tour</li>
                            <li className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-green-500 shrink-0" /> Email support</li>
                          </ul>
                        </div>
                        <Button 
                          ...
                          className={`w-full mt-5 font-bold ${
                            profile?.plan === 'basic' ? 'bg-gray-100 text-gray-400 cursor-default hover:bg-gray-100' : 'bg-[#0277bd] hover:bg-[#0266a1] text-white shadow-md'
                          }`}
                        >
                      ...
                      <div className={`border-2 rounded-2xl p-5 flex flex-col justify-between transition-all relative ${
                        profile?.plan === 'pro' 
                        ...
                        <div className="space-y-2">
                          <div className="text-base font-extrabold text-gray-800 flex items-center gap-1">
                            Pro Tier 
                            {profile?.plan === 'pro' && <CheckCircle2 className="h-4.5 w-4.5 text-[#8bc34a]" />}
                          </div>
                          <p className="text-[11px] text-gray-400 leading-snug">Designed for professional photographers and virtual tour agencies.</p>
                          <div className="pt-2 text-2xl font-black text-gray-900 flex items-baseline gap-0.5">
                            ₹1,499<span className="text-xs text-gray-400 font-bold">/mo</span>
                          </div>
                          <ul className="text-xs text-gray-500 space-y-1.5 pt-2 border-t font-medium">
                            <li className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-[#8bc34a] shrink-0" /> 3 users</li>
                            <li className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-[#8bc34a] shrink-0" /> 25 tours</li>
                            <li className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-[#8bc34a] shrink-0" /> 200 photos per tour</li>
                            <li className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-[#8bc34a] shrink-0" /> WhatsApp support</li>
                            <li className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-[#8bc34a] shrink-0" /> Priority processing</li>
                          </ul>
                        </div>
                        <Button 
                          ...
                          className={`w-full mt-5 font-bold ${
                            profile?.plan === 'pro' ? 'bg-gray-100 text-gray-400 cursor-default hover:bg-gray-100' : 'bg-blue-500 hover:bg-blue-600 text-white shadow-md'
                          }`}
                        >
                      ...
                      {/* Agency Plan card */}
                      <div className={`border rounded-2xl p-5 flex flex-col justify-between transition-all relative ${
                        profile?.plan === 'agency' ? 'border-[#0277bd] bg-blue-50/10 shadow-md ring-2 ring-[#0277bd]/20 scale-102' : 'hover:border-gray-300'
                      }`}>
                        <div className="space-y-2">
                          <div className="text-base font-extrabold text-gray-800">Agency Tier</div>
                          <p className="text-[11px] text-gray-400 leading-snug">Enterprise volume publishing. Unlimited tours, support, and custom setups.</p>
                          <div className="pt-2 text-2xl font-black text-gray-900 flex items-baseline gap-0.5">
                            ₹2,999<span className="text-xs text-gray-400 font-bold">/mo</span>
                          </div>
                          <ul className="text-xs text-gray-500 space-y-1.5 pt-2 border-t font-medium">
                            <li className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-green-500 shrink-0" /> Unlimited users</li>
                            <li className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-green-500 shrink-0" /> Unlimited tours</li>
                            <li className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-green-500 shrink-0" /> Unlimited photos</li>
                            <li className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-green-500 shrink-0" /> Dedicated support</li>
                            <li className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-green-500 shrink-0" /> White label option</li>
                          </ul>
                        </div>
                        <Button 
                          ...
                          className={`w-full mt-5 font-bold ${
                            profile?.plan === 'agency' ? 'bg-gray-100 text-gray-400 cursor-default hover:bg-gray-100' : 'bg-[#0277bd] hover:bg-[#0266a1] text-white shadow-md'
                          }`}
                        >
                  ...
                  <div className="space-y-3 pt-6 border-t">
                    <h3 className="text-sm font-bold text-gray-700 uppercase tracking-wider">Billing History</h3>
                          ...
                          {profile?.plan === 'trial' ? (
                            <tr>
                              <td colSpan={5} className="p-4 text-center text-gray-400 italic bg-gray-50/50">
                                No billing records found. Your invoices will compile here once you upgrade to a premium plan.
                              </td>
                            </tr>
                          ) : (
                            <tr className="hover:bg-slate-50/30">
                              ...
                              <td className="p-3 font-bold text-gray-800">
                                {profile?.plan === 'basic' ? '₹499' : profile?.plan === 'pro' ? '₹1,499' : '₹2,999'}
                              </td>
                    ...
                    <p className="text-[10px] text-gray-400 flex items-center gap-1.5 mt-1.5">
                      ...
                      18% GST invoice compiled automatically. For custom company GSTIN billing, contact help desk support.
                    </p>
                  </div>
                </div>
              )}
              ...
              {activeTab === "access" && (
                <div className="space-y-6 max-w-2xl">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">Review active OAuth authorization permissions granted to TourVista.</p>
                  </div>
                  ...
                  <div className="border rounded-2xl p-6 flex flex-col items-center justify-center text-center space-y-5 bg-slate-50/50">
                    ...
                    {isGoogleConnected ? (
                      <div className="space-y-4 w-full max-w-sm">
                        <div className="space-y-1">
                          ...
                          <p className="text-[10px] text-gray-400 px-4">
                            TourVista has active permissions to publish, retrieve, construct, and delete virtual tour scenes and constellation links under your Google Account directly on Google Maps.
                          </p>
                        </div>
                        <Button 
                      ...
                      <div className="space-y-4 w-full max-w-sm">
                        <div className="space-y-1">
                          <span className="text-gray-400 font-extrabold text-sm block">No Google Account Connected</span>
                          <p className="text-[10px] text-gray-400 px-4">
                            You must connect your Google Account to authorize TourVista to publish 360-degree scene uploads directly to Google Maps and Street View.
                          </p>
                        </div>
                        <Button 
              ...
              {activeTab === "support" && (
                <div className="space-y-6 max-w-2xl">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">Get in touch with the team or browse commonly asked support queries.</p>
                  </div>

                  <div className="bg-slate-50 border p-6 rounded-2xl flex flex-col md:flex-row items-center gap-6 justify-between">
                    <div className="space-y-1 flex-1">
                      <h3 className="text-base font-bold text-gray-700">Immediate WhatsApp Assistance</h3>
                      <p className="text-xs text-gray-400 leading-snug">Connect directly to a support representative. Mon–Sat 10:00 AM – 7:00 PM IST.</p>
                    </div>
                    <a 
                      href={waLink(`Hi TourVista Support, I need help with my account. Email is ${user?.email}`)}
                      target="_blank" 
                      rel="noreferrer"
                  ...
                  <div className="space-y-3 pt-4 border-t">
                    <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Frequently Asked Support FAQs</h3>
                    
                    <div className="rounded-xl border p-4 space-y-1 bg-white hover:bg-slate-50/20 transition-colors">
                      <h4 className="text-xs font-bold text-gray-700">How long does Google take to publish my scenes?</h4>
                      <p className="text-[11px] text-gray-400 leading-relaxed">
                        Google processes uploads asynchronously in the background. Typical time to display on Google Maps is around **2 to 24 hours**. Check standard processing using the **Sync Google Status** option inside your tour's publish section.
                      </p>
                    </div>

                    <div className="rounded-xl border p-4 space-y-1 bg-white hover:bg-slate-50/20 transition-colors">
                      <h4 className="text-xs font-bold text-gray-700">Can I update nadir sizing or logo after publishing?</h4>
                      <p className="text-[11px] text-gray-400 leading-relaxed">
                        Yes! Simply open your tour, go to the Publish tab, customize your Nadir blur, logo size, or position, and select **Reset & Delete from Google**. This removes stale spheres and lets you re-publish fresh scenes in a single click.
                      </p>
                    </div>

                    <div className="rounded-xl border p-4 space-y-1 bg-white hover:bg-slate-50/20 transition-colors">
                      <h4 className="text-xs font-bold text-gray-700">What billing configurations are supported?</h4>
                      <p className="text-[11px] text-gray-400 leading-relaxed">
                        Payments are processed securely via Razorpay, supporting UPI (GPay, PhonePe, Paytm), net banking, standard EMI credit/debit cards, and corporate corporate cards. Invoices will automatically compile 18% CGST/SGST.
                      </p>
                    </div>
