...
function Dashboard() {
  ...
  const isAdmin =
    user?.email === "panopublish@gmail.com" ||
        ...
        <div className="lg:col-span-2 rounded-xl border bg-card p-5">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm text-muted-foreground">
              {tourCount}/{limit === 9999 ? "∞" : limit} tours used
            </span>
          </div>
          ...
          <p className="text-xs text-muted-foreground mt-2">
            Plan: <strong className="capitalize">{profile?.plan ?? "trial"}</strong>
          </p>
        </div>
          ...
          <div className="rounded-xl border bg-card p-5">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold flex items-center gap-2">
                <ListChecks className="h-4 w-4 text-primary" /> Get started
              </h3>
              <button
            ...
            <p className="text-xs text-muted-foreground mb-3">
              {doneCount} of {onboarding.length} steps complete
            </p>
            <ul className="space-y-2">
              {onboarding.map((s) => (
                <li key={s.label} className="flex items-center justify-between gap-2 text-sm">
                  <span
                    className={`flex items-center gap-2 ${s.done ? "text-muted-foreground line-through" : ""}`}
                  >
                    <span
                      className={`inline-flex h-4 w-4 items-center justify-center rounded-full text-[10px] ${s.done ? "bg-success text-white" : "border"}`}
                    >
                      {s.done ? "✓" : ""}
                    </span>
                    {s.label}
                  ...
                  {!s.done && (
                    <Link to={s.to} className="text-xs text-primary font-medium">
                      Do this now
                    </Link>
                  )}
                </li>
...
function Stat({
  ...
  return (
    <div className="rounded-xl border bg-card p-4">
      <div
        className={`mb-2 inline-flex h-9 w-9 items-center justify-center rounded-lg ${accentCls}`}
      >
