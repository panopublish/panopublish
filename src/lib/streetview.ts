export async function syncStreetViewConnections(
  supabaseClient: any,
  tourId: string,
  accessToken: string,
) {
  ...
  const formattedConnections = latestPhotos
    ...
    .map((p) => {
      ...
      return {
        ...
        connected_ids: Array.from(
          new Set((p.streetview_photo_id && svConnections[p.streetview_photo_id]) || []),
        ),
        pose: {
  ...
  if (formattedConnections.length > 0) {
    const { data: connData, error: connError } = await supabaseClient.functions.invoke(
      "streetview-publish",
      {
        body: {
    ...
    if (connData?.error || connData?.success === false)
      throw new Error(connData.error || "Failed to update connections");
