...
import type { CityTourData } from "@/lib/india-city-tours-data";

export function CityTourLanding({ page }: { page: CityTourData }) {
  const [openFaq, setOpenFaq] = useState<number | null>(null);
  const [activeFloor, setActiveFloor] = useState<"L0" | "L1" | "L2">("L0");

  const cityName = page.cityName || "India";
  const stateName = page.state || "India";

  const breadcrumbs = [
    { name: "Home", url: "https://panopublish.com/" },
    { name: page.heading || `Google Maps 360° Virtual Tour in ${cityName}`, url: `https://panopublish.com/${page.slug}/` },
  ];
  ...
  const defaultCategories = [
    {
      ...
      areas: "Prime Commercial Corridors",
    },
    {
      ...
      areas: "Hospitality & Airport Zones",
    },
    {
    ...
    },
    {
      ...
      areas: "Residential & Commercial Markets",
    },
    {
    ...
    },
    {
      ...
      areas: "Institutional & Academic Belts",
    },
    {
      ...
      areas: "Celebration Lawns & Highway Corridors",
    },
  ];

  const businessCategories = page.businessCategories && page.businessCategories.length > 0
    ? page.businessCategories
    : defaultCategories;
  ...
  const processSteps = [
    ...
    },
    {
      step: "06",
      title: "Publish",
      desc: "Authorize your Google account with OAuth and publish directly to your Google Business Profile via the Google Street View Publish API. Panoramas typically index within 24–48 hours.",
      icon: CheckCircle2,
    },
  ];

  const verifiedFeatures = [
    { title: "Direct Google Publish API", desc: "No manual app work or mobile uploads. Secure browser-based streaming directly to Google Maps servers." },
    { title: "Google Place ID Integration", desc: "Lookup any verified business in seconds and link your 360° photo spheres to the official map marker." },
    { title: "GPano XMP Metadata Injection", desc: "Automatic embedding of PoseHeadingDegrees, projection type, and camera dimensions into JPEG headers." },
    { title: "Visual Connection Builder", desc: "Draw navigation arrows between adjacent rooms visually with real-time compass yaw calibration." },
    { title: "Automated Nadir Branding", desc: "Apply clean circular nadir logo disks (512x512px transparent PNG) or smart blur over tripod shadows." },
    { title: "Multi-Level / Islands Support", desc: "Organize large properties by floor levels (L0, L1, L2) to display structured multi-story navigation." },
    { title: "Privacy Blur Protection", desc: "Client-side canvas blur to easily protect customer faces, vehicle license plates, or confidential documents." },
    { title: "Client Workspace Management", desc: "Separate folders and settings for every client. Perfect for freelance 360° photographers and media agencies." },
    { title: "Flat INR Pricing (No USD Fees)", desc: "Predictable domestic billing via Razorpay with instant UPI, Net Banking, and corporate credit cards." },
  ];

  const costFactors = [
    { factor: "Number of Viewpoints / Nodes", why: "More photo spheres mean more physical capture positions, HDR bracketed shots, and connection links.", example: "A 6-node dental clinic vs a 35-node boutique hotel" },
    { factor: "Property Layout & Floor Count", why: "Multi-level buildings require floor-to-floor waypoint alignment, elevator linking, and level grouping.", example: "Single-story cafe vs 3-story wedding banquet facility" },
    { factor: "Lighting & Window HDR Blending", why: "Balancing bright natural outdoor sunlight through glass windows with interior ambient lighting.", example: "Showrooms with full glass facades" },
    { factor: "Custom Nadir Tripod Branding", why: "Designing and stamping custom circular agency/business logo disks over the camera tripod footprint.", example: "Branded 512x512 transparent PNG logo disk" },
    { factor: "Turnaround Time Requirements", why: "Rush deliveries (24–48 hours) usually command premium rates compared to standard 5–7 day delivery.", example: "Grand opening or time-sensitive product launch" },
  ];

  const commercialHubs = page.commercialHubs || [];
  ...
  const schemas = [
    {
      ...
      contentUrl: `https://panopublish.com${page.image}`,
      caption: page.imageAlt || `Google Maps 360° virtual tour in ${cityName}`,
      width: 1536,
      height: 864,
    },
    {
      "@context": "https://schema.org",
      "@type": "WebPage",
      "@id": `https://panopublish.com/${page.slug}/#webpage`,
      url: `https://panopublish.com/${page.slug}/`,
      name: page.title,
      ...
      about: {
        "@type": "Service",
        name: `Google Maps 360 Virtual Tour Publishing in ${cityName}`,
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col font-sans antialiased text-foreground">
      <SEO
        ...
        ogImage={`https://panopublish.com${page.image}`}
        schema={schemas}
      />
      ...
      <main className="flex-1">
          ...
          <div className="container relative mx-auto px-4 max-w-5xl">
            <div className="flex flex-col items-center text-center gap-5">
              <span className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-sky-400/25 bg-sky-400/10 text-xs font-semibold text-sky-300">
                <MapPin className="h-3.5 w-3.5 text-sky-400" /> {cityName} Local Business &amp; Photographer Resource
              </span>

              {/* EXACT SINGLE H1 */}
              <h1 className="text-3xl sm:text-4xl md:text-6xl font-extrabold tracking-tight font-serif leading-[1.15] max-w-4xl">
                {page.heading || `Google Maps 360° Virtual Tour in ${cityName}`}
              </h1>

              <p className="text-lg md:text-2xl text-sky-200/90 font-medium max-w-2xl font-serif">
                {page.subheading || "Let customers explore your business before they visit."}
              </p>

              <p className="text-sm md:text-base text-slate-300 max-w-2xl leading-relaxed">
                {page.introText || `Connected 360° panoramas give potential customers an interactive way to explore commercial properties across ${cityName}.`}
              </p>
                ...
                <div className="relative rounded-2xl md:rounded-3xl overflow-hidden border border-slate-800/90 shadow-2xl bg-slate-900/60 p-2 md:p-3 group">
                  ...
                  <picture className="block w-full aspect-[16/9] rounded-xl md:rounded-2xl overflow-hidden relative">
                    <source
                      ...
                      srcSet={page.imageMobile || page.image}
                      type="image/webp"
                    />
                    <img
                      src={page.image}
                      alt={page.imageAlt || `Google Maps 360° virtual tour in ${cityName}`}
                      width={1536}
                      height={864}
                      loading="eager"
                      fetchPriority="high"
                  ...
                  <div className="mt-3 px-2 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-400">
                    <span className="flex items-center gap-1.5">
                      ...
                      360° virtual tour camera capture &amp; interactive navigation in a modern {cityName} commercial property
                    </span>
            ...
            <div className="relative rounded-2xl overflow-hidden border border-slate-700 shadow-2xl bg-slate-950 p-2 md:p-3">
              <div className="relative aspect-video md:aspect-[21/9] w-full rounded-xl overflow-hidden bg-slate-900">
                <iframe
                  ...
                  title={`Live Google Street View 360° Tour Demonstration - ${cityName}`}
        ...
        {/* SECTION 4: WHY CITY BUSINESSES USE 360° TOURS */}
        <section className="py-16 md:py-24 bg-slate-50 border-b">
          <div className="container mx-auto px-4 max-w-5xl">
            <div className="text-center max-w-3xl mx-auto mb-16">
              ...
              <h2 className="text-2xl md:text-4xl font-bold font-serif text-slate-900 mt-2">
                Why {cityName} Businesses Use 360° Tours
              </h2>
              <p className="mt-3 text-slate-600 text-sm md:text-base">
                Different commercial sectors across {cityName} rely on connected 360° photography to provide transparency, answer customer questions, and build credibility.
              </p>
            </div>

            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {businessCategories.map((cat, i) => {
                const IconComponent = categoryIcons[i % categoryIcons.length] || Building2;
                return (
                  <div
                    key={i}
        ...
        {/* SECTION 5: LOCAL COMMERCIAL CONTEXT */}
        <section className="py-16 md:py-24 bg-white border-b">
          <div className="container mx-auto px-4 max-w-5xl">
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <h2 className="text-2xl md:text-4xl font-bold font-serif text-slate-900">
                  Supporting Businesses &amp; Photographers Across {cityName}
                </h2>
                <p className="mt-4 text-slate-600 text-sm md:text-base leading-relaxed">
                  {page.introText || `${cityName}'s commercial ecosystem spans dynamic retail high streets, industrial corridors, and corporate centers.`}
                </p>
                <p className="mt-3 text-slate-600 text-sm md:text-base leading-relaxed">
                ...
                <div className="mt-6 space-y-2.5 text-xs md:text-sm text-slate-700 font-medium">
                  <div className="flex items-center gap-2">
                    ...
                    <span>Works with any verified Google Business Profile in {cityName}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Check className="h-4 w-4 text-emerald-600 shrink-0" />
          ...
          <div className="container mx-auto px-4 max-w-5xl">
                ...
                {pricingTiers.map((tier, i) => (
                  <div key={i} className="p-6 rounded-2xl bg-slate-50 border border-slate-200 flex flex-col justify-between">
                    <div>
                      ...
                      <div className="text-2xl font-extrabold font-serif text-slate-900 mb-3">
                        {tier.typicalRange}
                      </div>
            ...
            <div className="p-6 rounded-2xl bg-amber-50/60 border border-amber-200 text-xs md:text-sm text-amber-900">
              ...
              <p className="leading-relaxed">
                Publicly listed provider prices can vary across {cityName}, so businesses should compare what is included (such as exposure bracketing, custom nadir tripod logo branding, line-of-sight node linking, and post-publish support) rather than choosing solely by headline price.
              </p>
            </div>
          </div>
        </section>
            ...
            <div className="grid md:grid-cols-3 gap-8">
              ...
              <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-xs flex flex-col justify-between">
                <div>
                  ...
                  <h3 className="text-lg font-bold font-serif text-slate-900 mb-2">
                    Commercial Dining / Cafe in {cityName}
                  </h3>
                  <p className="text-xs text-slate-600 mb-4 leading-relaxed">
                    12 connected panoramas covering entrance, reception, dining floor, and private dining cabins.
                  </p>
                  <ul className="space-y-2 text-xs text-slate-600 mb-6">
                    <li className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-emerald-600" /> Shot with Ricoh Theta Z1 / Insta360 HDR</li>
              ...
              <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-xs flex flex-col justify-between">
                <div>
                  ...
                  <h3 className="text-lg font-bold font-serif text-slate-900 mb-2">
                    Multi-Level Hotel / Facility in {cityName}
                  </h3>
                  <p className="text-xs text-slate-600 mb-4 leading-relaxed">
                  ...
                  <ul className="space-y-2 text-xs text-slate-600 mb-6">
                    ...
                    <li className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-emerald-600" /> Privacy blur on customer faces &amp; desks</li>
              ...
              <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-xs flex flex-col justify-between">
                <div>
                  ...
                  <h3 className="text-lg font-bold font-serif text-slate-900 mb-2">
                    Retail Showroom / Dealership in {cityName}
                  </h3>
                  <p className="text-xs text-slate-600 mb-4 leading-relaxed">
                    18 panoramas spanning entrance drive, customer lounge, display aisles, and service bays.
                  </p>
                  <ul className="space-y-2 text-xs text-slate-600 mb-6">
        ...
        <section className="py-16 md:py-24 bg-white border-b">
          <div className="container mx-auto px-4 max-w-4xl">
            <div className="text-center max-w-2xl mx-auto mb-16">
              ...
              <p className="mt-3 text-slate-600 text-sm md:text-base">
                Factual, clear answers about Google Maps 360° virtual tours in {cityName} and PanoPublish.
              </p>
            </div>
        ...
        <section className="py-20 md:py-28 bg-slate-950 text-white relative overflow-hidden">
          ...
          <div className="container relative mx-auto px-4 max-w-4xl text-center">
            ...
            <p className="mt-2 text-slate-400 text-sm md:text-base max-w-2xl mx-auto">
              Whether you are a local {cityName} business showcasing your venue or a commercial 360° photographer managing multiple client accounts, PanoPublish gives you a fast, reliable publishing studio.
            </p>
        ...
        {/* SECTION 17: RELATED CITIES & INTERNAL LINKS */}
        <section className="py-16 bg-white border-b">
          <div className="container mx-auto px-4 max-w-5xl">
            {relatedCities.length > 0 && (
              <div className="mb-10">
                <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400 mb-4 text-center md:text-left">
                  Explore Google Maps 360° Tours in Nearby Cities
                </h3>
                <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-3 text-xs md:text-sm">
                  {relatedCities.map((c, i) => (
                    <Link
                      key={i}
                      to="/$slug/"
                      params={{ slug: c.slug }}
                      className="p-3 rounded-xl border border-slate-200 hover:border-primary/50 hover:bg-slate-50 transition-colors font-medium text-slate-800 flex items-center justify-between"
                    >
                      <span>360° Tour in {c.name}</span>
